...
def create_general_prescription_analysis(title: str, description: str, model, generation_config: Dict) -> Dict:
    """Create a general prescription analysis when no specific medicines are found"""
    try:
        # Generate analysis directly from prescription text
        analysis_prompt = f"""
        Analyze the following prescription and provide a comprehensive medical analysis:
        
        Title: {title}
        Prescription Details: {description}
        
        Provide a structured analysis including:
        1. Summary of the prescription content
        2. Key findings and observations
        3. Important warnings or considerations
        4. General recommendations for the patient
        
        Format the response as a clear, professional medical analysis.
        """
        
        try:
            response = model.generate_content(
                [
                    "You are a medical AI assistant. Analyze prescription text and provide comprehensive medical insights, even if specific medicine names are not clearly identified.",
                    analysis_prompt
                ],
                generation_config=generation_config,
            ...
            error_msg = str(e)
            print(f"WARNING Error generating general prescription analysis: {error_msg}")
            # Fallback analysis
            analysis_text = f"Prescription Analysis for: {title}\n\n{description}\n\nPlease consult your healthcare provider for detailed medication information and instructions."
        
        # Create simplified summary
        simplified_summary = analysis_text[:500] if len(analysis_text) > 500 else analysis_text
        if len(analysis_text) > 500:
            simplified_summary = simplified_summary.rsplit(' ', 1)[0] + "..."
        
        return {
            "summary": f"Prescription analysis for {title}. Review the prescription details and consult your healthcare provider for specific medication instructions and dosage information.",
            "simplifiedSummary": simplified_summary,
            "keyFindings": [
                "Prescription document received and analyzed",
                "Review prescription details carefully",
                "Consult healthcare provider for medication instructions",
                "Verify dosage and administration schedule"
            ],
            "riskWarnings": [
                "WARNING Always verify prescription details with your healthcare provider",
                "WARNING Follow medication instructions exactly as prescribed",
                "WARNING Report any adverse effects immediately",
                "WARNING Do not share medications with others"
            ],
            "recommendations": [
                "* *Medical Consultation* - Review this prescription with your healthcare provider or pharmacist",
                "* *Medication Adherence* - Follow the prescription instructions exactly as provided",
                "* *Monitoring* - Monitor for any side effects or adverse reactions",
                "* *Follow-up* - Schedule follow-up appointments as recommended",
                "* *Safety* - Store medications properly and keep out of reach of children"
            ],
            "confidence": 0.70,  # Lower confidence since no specific medicines identified
            "analysisType": "General Prescription Analysis",
        ...
        # Ultimate fallback
        return {
            "summary": f"Prescription received: {title}. Please review the prescription details and consult your healthcare provider for medication instructions.",
            "simplifiedSummary": description[:500] if len(description) > 500 else description,
            "keyFindings": ["Prescription document received", "Review with healthcare provider"],
            "riskWarnings": ["WARNING Always verify prescription details with your healthcare provider"],
            "recommendations": ["* *Medical Consultation* - Review this prescription with your healthcare provider"],
            "confidence": 0.60,
            "analysisType": "Basic Prescription Analysis",
            "aiDisclaimer": "⚠️ **AI Analysis Disclaimer**: This prescription analysis is generated by AI and is for informational purposes only.",
            "detailedReport": f"Prescription: {title}\n\n{description}"
        }
...
def analyze_health_record_with_ai(record_data: Dict) -> Dict:
    ...
    try:
        ...
        if record_type == 'prescription':
            ...
            # Improved prompt to handle various prescription formats
            try:
                response = model.generate_content(
                    [
                        "You are MedGuide AI. Extract ALL medicine names, drug names, medication names, or pharmaceutical names from the prescription text. "
                        "Look for any medication mentioned, even if written in different formats (brand names, generic names, abbreviations). "
                        "If no medicines are found, return an empty array: []. "
                        "Return ONLY a JSON array of medicine names found in the prescription. "
                        "Example: [\"Medicine1\", \"Medicine2\", \"Medicine3\"] or [] if none found",
                        f"Prescription Text: {title}\n\n{description}"
                    ],
                    generation_config=generation_config,
                    request_options={'timeout': 60}  # 60 second timeout for API call
                )
            ...
            if not medicine_names:
                ...
                common_medicines = ['amoxicillin', 'metformin', 'lisinopril', 'aspirin', 'ibuprofen', 'acetaminophen', 
                                   'paracetamol', 'diclofenac', 'omeprazole', 'atorvastatin', 'amlodipine', 'levothyroxine',
                                   'gabapentin', 'sertraline', 'tramadol', 'prednisone', 'furosemide', 'warfarin']
                ...
                if fallback_medicines:
                    ...
                    # Even if no medicines found, create a basic analysis from the prescription text
                    print("INFO No specific medicines found, creating general prescription analysis")
                    # Use the prescription text itself to generate a meaningful analysis
                    return create_general_prescription_analysis(title, description, model, generation_config)
